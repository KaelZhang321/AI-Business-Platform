...
def test_trigger_incremental_governance_returns_accepted_job(monkeypatch) -> None:
    """触发接口只返回任务句柄，不在 HTTP 请求里同步等待治理完成。"""

    stub_service = _StubGovernanceJobService()
    monkeypatch.setattr(governance_routes, "_get_governance_job_service", lambda: stub_service)
...
def test_get_incremental_governance_job_returns_snapshot(monkeypatch) -> None:
...
def test_trigger_cold_start_governance_returns_accepted_job(monkeypatch) -> None:
    """冷启动治理触发也必须异步返回任务句柄。"""
...
def test_get_incremental_governance_job_returns_404_when_missing(monkeypatch) -> None:
...
def test_get_curation_run_returns_snapshot(monkeypatch) -> None:
    """run 查询应返回控制面快照。"""

    stub_repo = _StubRunRepository()
    monkeypatch.setattr(governance_routes, "_get_curation_run_repository", lambda: stub_repo)
...
def test_promote_curation_run_calls_publication_service(monkeypatch) -> None:
    """发布接口应转发到发布服务执行原子切流。"""

    stub_repo = _StubRunRepository()
    stub_publication = _StubPublicationService(stub_repo.snapshot)
    monkeypatch.setattr(governance_routes, "_get_publication_service", lambda: stub_publication)
...
def test_rollback_curation_run_calls_publication_service(monkeypatch) -> None:
    """回滚接口应调用发布服务恢复目标 run。"""
