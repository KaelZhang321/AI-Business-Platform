...
本设计新增客户个人信息固定分支：当 query 命中客户档案查询语义时，固定读取 `ai-gateway/resources/ui_api_endpoints.xlsx` 中 `status='active'` 的 endpoint 元数据，先定位客户，再调用客户档案相关接口，最后返回固定顺序的 `PlannerCard + PlannerDetailCard/PlannerInfoGrid/PlannerTable` UI Spec。
...
4. UI 顺序严格按业务图片大标题顺序展示。
5. 普通分区卡片统一使用 `PlannerDetailCard`。
6. 资产概览使用 `PlannerInfoGrid`，资产明细使用 `PlannerTable`。
7. 单个分区失败不破坏整体档案结构，返回 `PARTIAL_SUCCESS` 并保留失败分区占位。
...
3. 执行客户主查询。
4. 主查询成功后，对其他 active endpoint 生成下游 steps。
5. 下游 steps 统一依赖 `step_get_customer_info`。
6. 参数绑定优先消费 endpoint 的 `predecessor_specs`。
7. 常见绑定为从客户主查询结果的 `idCard` 绑定到下游接口的 `encryptedIdCard`。
8. 下游步骤并发执行。
...
4. 客户主查询是否成功。
5. 下游成功、失败、空结果分区数量。
6. 参数绑定来源：`predecessor_specs`、兜底绑定或绑定失败。
...
16. `list[string]` 字段能用 `、` 拼接展示。
17. 非客户档案查询继续走原 `/api-query` 通用流程。
...
3. 新增固定 DAG 构造器，复用现有 `ApiQueryExecutionPlan` 和 `ApiQueryPlanStep`。
4. 新增固定 UI spec 构造器，复用 `PlannerCard`、`PlannerDetailCard`、`PlannerInfoGrid`、`PlannerTable`。
5. 在 `ApiQueryWorkflow` 的 route/plan 分支中接入，避免污染普通查询路径。
