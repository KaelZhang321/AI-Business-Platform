    ...
    <section className="xl:col-span-3 bg-white/40 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col">
      <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
        <div className="flex items-center space-x-8">
          <button 
            ...
            className={`text-lg font-bold flex items-center transition-colors relative ${activeTab === 'work' ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}
          >
            我的工作
          </button>
          <button 
            onClick={() => setActiveTab('todo')}
            className={`text-lg font-bold flex items-center transition-colors relative ${activeTab === 'todo' ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}
          >
            待办事项
        ...
        <div className="flex items-center space-x-3">
          <button className="flex items-center px-3 py-1.5 bg-slate-50 text-slate-600 text-sm font-medium rounded-xl hover:bg-slate-100 transition-colors border border-slate-200 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 relative z-10 items-start">
          ...
          return (
            ...
            >
              ...
              <div className="bg-slate-50/50 rounded-xl p-3 mb-4 border border-slate-100">
                <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">任务描述</div>
              ...
              <div className="mt-auto pt-2 flex flex-col space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">执行进度</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-slate-900">{todo.progress}%</span>
        ...
        <button
          ...
          className="group flex flex-col items-center justify-center p-8 rounded-2xl bg-slate-50/50 border-2 border-dashed border-slate-200 cursor-pointer hover:bg-white hover:border-brand/40 transition-all duration-300 min-h-[280px]"
        >
          <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-sm group-hover:shadow-md group-hover:border-brand/20">
