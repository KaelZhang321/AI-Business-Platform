  ...
  return (
    <div className="space-y-10 pb-12 text-slate-900 dark:text-slate-100">
      ...
      <section className="relative py-16 px-8 rounded-[32px] overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-brand/10 via-brand/5 to-transparent dark:from-brand/20 dark:via-brand/10 dark:to-slate-900/0 -z-10"></div>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">通过AI赋能临床决策，解决诊断与方案规划核心挑战，提升医疗服务质量</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {FUNCTION_MODULES.latest.map((item, idx) => (
            <motion.button
              ...
              className="group bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl dark:hover:shadow-none transition-all border border-slate-100 dark:border-slate-700 cursor-pointer text-left"
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">探索智能驾驶舱之上的医疗助手，提高诊疗效率，提升患者康复体验</p>
          </div>
          <button className="text-brand font-bold flex items-center hover:underline text-sm">
            查看全部 <ChevronRight className="w-4 h-4 ml-0.5" />
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FUNCTION_MODULES.recommended.map((item, idx) => (
            <motion.button
              ...
              className="flex items-start p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-md dark:hover:shadow-none transition-all border border-slate-50 dark:border-slate-700 group cursor-pointer"
              aria-label={item.title}
              onClick={() => handleModuleClick(item.title)}
            >
              ...
              <div className="space-y-1 min-w-0">
                <div className="flex items-center space-x-2">
                  <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm group-hover:text-brand transition-colors truncate">{item.title}</h4>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">汇聚辅助诊断、干预方案、病历生成、报告解读等全方位医疗AI工具</p>
          </div>
          <button className="text-brand font-bold flex items-center hover:underline text-sm">
            查看全部 <ChevronRight className="w-4 h-4 ml-0.5" />
          </button>
          ...
          {FUNCTION_MODULES.all.map((item, idx) => (
            ...
            >
              ...
              <div className="space-y-1 min-w-0">
                <div className="flex items-center space-x-2">
                  <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm group-hover:text-brand transition-colors truncate">{item.title}</h4>
                  <span className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 text-[10px] rounded font-bold uppercase tracking-wider shrink-0">{item.tag}</span>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1">{item.desc}</p>
              </div>
            </motion.button>
      ...
      {/* Footer Banner */}
      <div className="bg-brand/5 dark:bg-brand/10 rounded-[32px] p-8 flex flex-col md:flex-row items-center justify-between border border-brand/10 dark:border-brand/20">
        <div className="flex items-center space-x-4 mb-4 md:mb-0">
          <div className="text-3xl">🤔</div>
          <div>
            <h4 className="font-bold text-slate-900 dark:text-slate-100">没有找到合适的功能或方案？</h4>
            <p className="text-sm text-slate-500 dark:text-slate-400">提交需求，享专人服务</p>
          </div>
        </div>
