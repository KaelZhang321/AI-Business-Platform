...
# ===== 提取模块专属变量，覆盖公共变量 =====
# 例如 MODULE_BACKEND_IMAGE_NAME=crm-service → IMAGE_NAME=crm-service
while IFS= read -r line || [[ -n "$line" ]]; do
  [[ -z "$line" || "$line" =~ ^[[:space:]]*# ]] && continue
  key="${line%%=*}"
  value="${line#*=}"
...
if [ -z "$BUILD_CMD" ]; then
  if [ -f "pom.xml" ]; then
    echo "Detected Maven project. Setting build command to 'mvn clean package -DskipTests'."
...
REMOTE_SCRIPT="${REMOTE_SCRIPT:-./skills/backend-deploy/scripts/remote_deploy_template.sh}"
...
if [ "$GIT_COMMIT_ENABLED" = "true" ]; then
    ...
    echo "Git commit disabled in configuration. Skipping..."
fi
...
execute_remote() {
    ...
    local env_vars="export ALIYUN_REGISTRY='$ALIYUN_REGISTRY'; export ALIYUN_NAMESPACE='$ALIYUN_NAMESPACE'; export ALIYUN_USER='$ALIYUN_USER'; export ALIYUN_PASSWORD='$ALIYUN_PASSWORD'; export IMAGE_NAME='$IMAGE_NAME'; export IMAGE_TAG='$IMAGE_TAG'; export FULL_IMAGE_NAME='$FULL_IMAGE_NAME'; export REMOTE_DIR='$REMOTE_DIR'; export PROFILES_ACTIVE='$PROFILES_ACTIVE';"
